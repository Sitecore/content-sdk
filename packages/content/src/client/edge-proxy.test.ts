...
describe('edge-proxy', () => {
  describe('getEdgeProxyContentUrl', () => {
    it('should return url', () => {
      ...
      expect(url).to.equal(`${SITECORE_EXPERIENCE_EDGE_URL_DEFAULT}/v1/content/api/graphql/v1`);
    });
  ...
  describe('getEdgeProxyFormsUrl', () => {
    ...
    it('should return url', () => {
      ...
      expect(url).to.equal(
        `${SITECORE_EXPERIENCE_EDGE_URL_DEFAULT}/v1/forms/publisher/${formId}?sitecoreContextId=${sitecoreEdgeContextId}`
      );
    });
