...
describe('generateMap', () => {
  ...
  let generateMap: GenerateMapFunction;
  ...
  beforeEach(async () => {
    ...
    generateMap = (await import('./generate-map.js')).generateMap;
  });
  ...
  it('should write componentMap.ts file with components from "paths" parameter', async () => {
    ...
    const [dest, content] = writeFileSyncStub.getCall(0).args;
  ...
  it('should use template from custom componentMap function, when provided', async () => {
    ...
    expect(writeFileSyncStub).to.have.been.calledOnce;
    const [, content] = writeFileSyncStub.getCall(0).args;
  ...
  it('should generate an empty component map if no components are found', async () => {
    ...
    expect(writeFileSyncStub).to.have.been.calledOnce;
    const [, content] = writeFileSyncStub.getCall(0).args;
    expect(content).to.include('export const componentMap = new Map<string, NextjsJssComponent>(');
