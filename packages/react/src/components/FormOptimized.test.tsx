...
import { FormOptimized, mockFormModuleOptimized } from './FormOptimized';
import sinon from 'sinon';
import { PageMode } from '@sitecore-content-sdk/content/client';
import { LayoutServicePageState } from '@sitecore-content-sdk/content/layout';

describe('FormOptimized', () => {
  const ctx = {
    api: {
      edge: {
        contextId: 'server-id',
  ...
  it('renders form using clientContextId when both IDs are present', async () => {
    ...
    const rendered = await render(
      <SitecoreProvider api={ctx.api} page={ctx.page.normal}>
        <FormOptimized rendering={rendering} params={rendering.params} />
      </SitecoreProvider>,
