...
describe('<TextOptimized />', () => {
  it('should render nothing with missing field', () => {
    const field: TextField = null;
    const rendered = render(<TextOptimized field={field} />);
    expect(rendered.container.innerHTML).to.equal('');
  });

  it('should render nothing with empty string value', () => {
    const field = {
      value: '',
    };
    const rendered = render(<TextOptimized field={field} />);
  ...
  it('should render value with editing explicitly disabled', () => {
    ...
    const rendered = render(
      <TextOptimized tag="span" field={field} editable={false} />
  ...
  it('should render value with just a value', () => {
    ...
    const rendered = render(<TextOptimized tag="span" field={field} />).container.querySelector(
      'span'
    );
    expect(rendered.innerHTML).to.contain('value');
  });
  ...
  it('should render embedded html as-is when encoding is disabled', () => {
    ...
    const rendered = render(<TextOptimized field={field} encode={false} />).container.querySelector(
      'span'
    );
  ...
  it('should render tag with a tag provided', () => {
    ...
    const rendered = render(<TextOptimized field={field} tag="h1" />).container.querySelector('h1');
    expect(rendered?.innerHTML).to.contain('value');
  });
  ...
  it('should render other attributes with other props provided', () => {
    ...
    const rendered = render(
      <TextOptimized field={field} tag="h1" className="cssClass" id="lorem" />
  ...
  describe('edit mode', () => {
    ...
    it('should render custom empty field component when provided, when field value is empty in edit mode metadata', () => {
      ...
      const rendered = render(
        <TextOptimized field={field} emptyFieldEditingComponent={EmptyFieldEditingComponent} />
      );
    ...
    it('should render nothing when field value is empty, when editing is explicitly disabled in edit mode metadata ', () => {
      ...
      const rendered = render(<TextOptimized field={field} editable={false} />);
    ...
    it('should apply suppressHydrationWarning prop when in editing mode', () => {
      ...
      const rendered = render(<TextOptimized field={field} tag="span" />);
      
      // Check that the rendered output contains the value
      // The suppressHydrationWarning is applied via JSX now
      const span = rendered.container.querySelector('span');
      expect(span).to.not.be.null;
      expect(span?.innerHTML).to.contain('value');
    });
  });
});
