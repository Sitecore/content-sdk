...
- **Personalization:** `createPersonalizeMiddleware(...)` calls Sitecore CDP and writes `variantId` / `componentVariantIds` / trusted `tokens` onto `req.scParams`; `pageLoader` forwards variants to `getPage(..., { personalize, deferFinalization: true })` and `finalizePageLoader` applies `getPersonalizeTokens(context) ?? {}`. It skips `/api/*`, editing and preview, bots, and prefetch requests (`x-sc-purpose: prefetch`) — a prefetch must never fire a CDP exposure event. A CDP timeout falls back to the default variant rather than blocking the render. Requires Edge configuration (`contextId` / `clientContextId`) and does not work against local containers. Responses with usable visitor token values set `Cache-Control: private, no-store`. Variant-only success does not add that restriction. Browser-side CDP event tracking is not implemented.
