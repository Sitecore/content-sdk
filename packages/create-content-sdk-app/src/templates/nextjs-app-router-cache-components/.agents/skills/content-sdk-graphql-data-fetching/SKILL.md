...
- **Segment not-found:** `getCachedPageParams()` → static HTML when site is empty or `isBuildValidationSite` matches **site or locale**; otherwise `getSitecoreErrorPage`. Do not call `headers()` in not-found.
