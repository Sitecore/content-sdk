...
- Single Sitecore page: `src/app/[site]/[locale]/[[...path]]/page.tsx`. Use `await params` for `{ site, locale, path? }`; if `draftMode().isEnabled` use the client directly, otherwise call `getSitecorePage({ site, locale, path: path ?? [] })`. Call `setRequestLocale(\`${site}_${locale}\`)` at the top. Layout chain: `app/layout.tsx` → `app/[site]/layout.tsx` (Bootstrap, draftMode) → `app/[site]/[locale]/[[...path]]/layout.tsx` (calls `setCachedPageParams({ site, locale })`) → page. Segment not-found: `src/app/[site]/[locale]/[[...path]]/not-found.tsx` reads `getCachedPageParams()` and calls `getSitecoreErrorPage`. Root not-found: `src/app/not-found.tsx` falls back to `scConfig.defaultSite` / `scConfig.defaultLanguage` and calls `getSitecoreErrorPage`.
...
- **Segment not-found (404):** `src/app/[site]/[locale]/[[...path]]/not-found.tsx`. Reads `{ site, locale }` via `getCachedPageParams()` (set by the segment layout for the same request), with `scConfig.defaultSite` / `scConfig.defaultLanguage` as a safety fallback, then calls `getSitecoreErrorPage({ site, locale, code: ErrorPage.NotFound })`. Wrap the rendered layout in `NextIntlClientProvider` so i18n works in the 404 markup. **Do not** call `headers()` here — it would opt the route out of SSG; the `getCachedPageParams` mechanism exists precisely to avoid that.
- **Root not-found (fallback 404):** `src/app/not-found.tsx`. Used when no segment claims the request (e.g. an unknown top-level path). Resolves site/locale from `scConfig.defaultSite` / `scConfig.defaultLanguage` and calls `getSitecoreErrorPage(...)` so 404 content gets the same Sitecore cache tags (`sc:route`, `sc:item`, `sc:pvv`) as a normal page and can be invalidated by webhook.
