...
# Content SDK Upgrade Assistant (App Router + Cache Components)
...
- Bump all @sitecore-content-sdk/* to consistent versions; read the Content SDK repo CHANGELOG (and MIGRATION/upgrade docs) for breaking changes and migration steps. Update package.json, run `npm install` and `npm run build`; test editing, preview, and an on-demand revalidate call after upgrade.
...
- Stop if a breaking change requires product or deployment decisions (e.g. new env vars, config schema, new tag scheme); list required changes and ask the user to confirm.
