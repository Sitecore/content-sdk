...
- **Page component:** `src/app/[site]/[locale]/[[...path]]/page.tsx`. Receives `params: Promise<{ site, locale, path? }>`. Use `await params`. Call `loadSitecoreRoutePage({ site, locale, path, searchParams })` from `src/lib/cache/get-sitecore-page.ts` so the page body and `generateMetadata` share one request-scoped result (draft uses preview / Design Library; published pages finalize tokens outside `'use cache'`).
...
- **Page data:** Use `loadSitecoreRoutePage` from `src/lib/cache/get-sitecore-page.ts`. Raw `getPage(..., { deferFinalization: true })` stays inside `'use cache'`. Visitor tokens are applied in a React `cache()` finalize helper so they never key the persistent cache. Preview / Design Library stay dynamic and omit tokens.
...
- **Webhook payload (`updates[]`):** Send the Sitecore Experience Edge / Content Operations body (`updates`, `invocation_id`, `continues`). The handler maps each `identifier` (with `-media` / `-layout` stripped) to `sc:item:<id>:<locale>` and revalidates it. A Dictionary entry update (`entity_definition: "DictionaryEntry"`) instead revalidates `sc:dict:<site>:<locale>` for the specific site resolved from the identifier via `sites` — not every configured site's.
